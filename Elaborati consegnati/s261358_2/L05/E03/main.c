...
void data(dati **v);
void tratta(dati **v);
void partenza(dati **v);
void arrivo(dati **v);
void stampa(dati dati);
void multi_ord(dati ***v,int comando);
int leggi_file(dati **v,dati ****ord);
...
int main()
{
    ...
    int comando=-1,i,j,giorno1,mese1,anno1,giorno2,mese2,anno2,data1,data2;
    char stringa[30];
    int l,N,r,m;
    int num_righe;
...
int leggi_file(dati **v,dati ****ord){
    ...
    tmp=malloc(num_righe*sizeof(dati));
    //ho 4 ordinamenti possibili.
    *ord=malloc(sizeof(dati **) * 4);
    if(*v==NULL || *ord==NULL) return -1;
    for(i=0;i<4;i++){
        (*ord)[i]=malloc(sizeof(dati *) * num_righe);
        if((*ord)[i]==NULL) return -1;
    }
    for(i=0;i<num_righe;i++){
        fscanf(fp,"%s %s %s %d:%d:%d %d %d %d",tmp[i].cod_tratta,tmp[i].partenza,tmp[i].destinazione,&tmp[i].giorno,&tmp[i].mese,
               &tmp[i].anno,&tmp[i].ora_partenza,&tmp[i].ora_arrivo,&tmp[i].ritardo);
        for(j=0;j<4;j++){
            (*ord)[j][i]=&tmp[i];
        }
    }
